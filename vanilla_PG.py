...
class agent():
    def __init__(self, lr, s_size,a_size,h_size):
        ...
        self.temp = tf.range(0, tf.shape(self.output)[0]) * tf.shape(self.output)[1]
        self.indexes = self.temp + self.action_holder
...
myAgent = agent(lr=learning_rate,s_size=4,a_size=2,h_size=8) #Load the agent.
    ...
    while i < total_episodes:
        ...
        for j in range(max_ep):
            ...
            if d == True:
                ...
                #rint(ep_history[:,0])
                #print(np.vstack(ep_history[:,0]))
                #print(ep_history[:,2])
                grads,lo = sess.run([myAgent.gradients,myAgent.loss], feed_dict=feed_dict)
                #print(temp)
                #print(indices)
        ...
        if i % 100 == 0:
            print(np.mean(total_reward[-20:]))
            re.append(np.mean(total_reward[-20:]))
        i += 1
